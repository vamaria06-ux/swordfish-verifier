...
def _write_config(tmp_path, specification_path, extra=""):
    ...
    config_file.write_text(f"""specification_path: "{specification_path}"
...
def test_missing_required_field_raises_error(tmp_path):
    config_file = tmp_path / "config.yaml"
    config_file.write_text(""" specification_path: "./specs/schemas" timeout: 10 """)
...
def test_specification_path_must_exist(tmp_path):
    """Если папки со схемами не существует — понятная ошибка, а не падение потом внутри parser.py"""
...
def test_specification_path_must_be_directory_not_file(tmp_path):
    """specification_path должен быть папкой со схемами, а не одним файлом
    (подтверждено параметром schemas_dir в UniversalSchemaParser.__init__)"""
