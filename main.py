...
def get_system_urls(client):
    ...
    """
    response = client.get("/redfish/v1/StorageServices")
    if response is None:
        return []
...
def main():
    ...
    resource_choice = menu("Что проверять?", [
        ...
        "Только базовые (ServiceRoot, Systems, StorageSystems, StoragePools, Volumes, Drives)",
        "Выбрать вручную"
    ])

    if resource_choice == 2:
        rules = {k: v for k, v in rules.items()
                 if k in ("ServiceRoot", "Systems", "StorageSystems", "StoragePools", "Volumes", "Drives")}
